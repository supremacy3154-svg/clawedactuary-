...
def render_blog_grid(posts: list[Post]) -> str:
    cards = ['<div class="articles-grid">']
    for post in posts:
        cards.append(
            f"""  <a class="article-card" href="{post.url}">
    ...
    <div class="a-foot">
      ...
      <span class="a-readtime">{post.reading_minutes} min read</span>
    </div>
  </a>"""
        )
    cards.append("</div>")
...
def write_features_json(cfg: dict) -> None:
    ...
    out = GENERATED_DIR / "site-features.json"
...
def main() -> int:
    ...
    sync_index(posts)
    sync_blog(posts)
    print(f"✓ Synced {len(posts)} posts → index.qmd, blog.qmd, _generated/site-features.json")
    return 0
